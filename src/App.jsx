...
const PolaroidStudio = () => {
  ...
  // NEW: Analog Pack Toggles
  const [showLeak, setShowLeak] = useState(false); // Light Leak
  const [showTape, setShowTape] = useState(false); // Washi Tape
  ...
  const handleSave = async () => {
    ...
    // 3. LIGHT LEAK (Canvas Implementation)
    if (showLeak) {
